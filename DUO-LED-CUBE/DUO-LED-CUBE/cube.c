...
void tmpsetcube(uint8_t value)
{
	for(int set_x=0;set_x<=8;set_x++)
		for(int set_y=0;set_y<=8;set_y++)
			for(int set_z=0;set_z<=8;set_z++)
				LED_tmp[set_z][set_x][set_y]=value;	
}
...
void effect_blinky2() // blinky power on
{
	int i;
	set_cube(0);
		i = 75;
...
void effect_box_shrink_grow (uint8_t rot, uint8_t flip, uint8_t delay)
{
	...
	for (uint8_t i=0;i<16;i++)
	{
        ...
        cli();
		box_wireframe(0,0,0,xyz,xyz,xyz,0xff);
...
void effect_box_woopwoop (int delay, int grow)
{
	...
	for (uint8_t i=0;i<4;i++)
	{
		...
		box_wireframe((3-ii),(3-ii),(3-ii),(4+ii),(4+ii),(4+ii),0);
		_delay_ms(delay);
	}
...
void effect_wormsqueeze (int size, int axis, int direction, int iterations, int delay)
{
	...
	for (i=0; i<iterations; i++)
	{
		...
		for (j=0; j<size;j++)
		{
			...
			{
				if (axis == AXIS_Z)
					setvoxel(x+j,y+k,origin,0xff);
					
				if (axis == AXIS_Y)
					setvoxel(x+j,origin,y+k,0xff);
					
				if (axis == AXIS_X)
					setvoxel(origin,y+j,x+k,0xff);
			}
		}
...
void setplane(uint8_t plane, uint8_t level)
{
	...
	for (i=0;i<8;i++)
		...
		{		
			switch (plane)
			{
			case AXIS_Z:	LED[level%8][i][j] = 0xff;
					break;
			case AXIS_Y:	LED[i][level%8][j] = 0xff;
					break;
			case AXIS_X:	LED[i][j][level%8] = 0xff;
					break;
			default: break;
			}
		}	
...
void effect_random_sparkle_flash (int iterations, int voxels,int value, int delay)
{
	...
	{
		for (v=0;v<=voxels;v++)
			setvoxel(rand()%8,rand()%8,rand()%8,value);
...
void effect_random_sparkle (int iterations, int max_voxels, int delay)
{
	...
	{
		effect_random_sparkle_flash(iterations,i,0xff,delay);
	}
	
	for (i=20;i>=1;i--)
	{
		effect_random_sparkle_flash(5,i,0xff,delay);
	}
